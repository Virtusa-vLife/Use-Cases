...
# Image input dataset folder path
DATA_DIR = os.path.join(CURRENT_PATH,'imagenet')
# Path to store result
MEM_DIR = os.path.join(CURRENT_PATH,'results')
# Path of InceptionV3 model checkpoint
CHECKPOINT = os.path.join(CURRENT_PATH,'inception_v3.ckpt')
# Default size of each Batch
BATCH_SIZE = 20
TIME_START = time.time()
# Number of class of image in input dataset.
NUM_CLASSES = 10
SAVE_FREQ = 10

# Responsible to remove the selected filters from InceptionV3 network
...
# This function will return output as tensor of filter in InceptionV3 network.
def return_player_output(model, player):
    '''The output of a filter.'''
    #player:InceptionV3/Mixed_7b/Branch_1/Conv2d_0a_1x1_73
    layer = '_'.join(player.split('_')[:-1])
    layer = '/'.join(layer.split('/')[1:])
    unit = int(player.split('_')[-1])
    return model.ends[layer][..., unit]#Tensor("strided_slice:0", shape=(?, 147, 147), dtype=float32)

# This function is a iteration of Neuoron Shapley Algorithm and return filters with their shapley value
def one_iteration(
    model, 
    players,
    images, 
    ...
    print('*'*10,'chosen_players length:{}'.format(len(chosen_players)))
    for n, idx in enumerate(idxs[::-1]):
        if idx in chosen_players:
            ...
            if isinstance(truncation, int):
                ...
                if n%10== 0:
                    print('N:{},Time:{},New value:{}'.format(n, time.time() - t, new_val))
                val_diff = new_val - base_value#base_value=0.001
                
                if metric == 'accuracy' and val_diff <= truncation:#truncation=0.2
...
#This function help to handle very big dataset input images with batchwise output. 
...
#Function responsible calculate performance of model for given image dataset and corresponding label
#for assign shapley value among neurons, so it return performance value of network.    
...
# Function to create iterative adversarial attack with Projected Gradient Descent.
...
# function for loading images of particular class of image dataset.
def load_images(files_dir, filenames, num_workers=0):
    ...
    if num_workers:
        ...
        images = pool.map(lambda f: np.array(PIL.Image.open(f)), file_dirs)
        pool.close()
    else:
        images = [np.array(PIL.Image.open(f)) for f in file_dirs]
    return np.array(images)/255

# Function to return the targeted image from the input image dataset.
def return_target_images(image_list, key):

    all_classes = list(set([image.split('/')[0] for image in image_list]))
...
#                      if filename.split('/')[-2] in target_classes]
                        if filename.split('/')[0] in target_classes]
    
                        
    return target_images

# Function to help in creating a adversarial images of given input images, so it return modified form of images.
...
# Function to load image and corresponding labels from image dataset, so it return loaded images and their labels.
def load_images_labels(key, num_images, max_sample_size, model, max_size=25000):

#     image_list = open('val_images.txt').read().split('\n')[:max_size]
    path=os.path.join(CURRENT_PATH,'pandas.txt')
    image_list = open(path).read().split('\n')[:max_size]
...
## Class name. Use 'all' for overll performance.
key = sys.argv[1] 
model_scope = 'InceptionV3'
#metric one of accuracy, binary, xe_loss.
metric = sys.argv[2]
## Number of validation images.
num_images = int(sys.argv[3]) 
bound = 'Bernstein'
## default truncation is 20%
truncation = 0.2
## Maximum number of images in one batch of sample.
max_sample_size = 128
#If True, computes contributions for adversarial setting.
adversarial = (sys.argv[4] == 'True') 
...
## Load Model using model checkpoint and find all convolutional filters
...
while True:
    ...
    # check for input metric is binary
    if metric == 'binary':
   ...
   # Getting players and their shapley value from Neuron Shapley algorithm.
    ...
    ## Save results every SAVE_FREQ iterations into .h5 file
    if counter % SAVE_FREQ == SAVE_FREQ - 1:#SAVE_FREQ=10
        with h5py.File(save_dir, 'w') as foo:
            foo.create_dataset("mem_tmc", data=mem_tmc, compression='gzip')
            foo.create_dataset("idxs_tmc", data=idxs_tmc, compression='gzip')
#             print('#'*50,'SAVE_FREQ H5 file','idxs:{},vals:{},mem_tmc:{},idxs_tmc:{}'.format(idxs,vals,mem_tmc,idxs_tmc))
            
    
    print('Experiment Time:{},Total Time:{}'.format(time.time() - t_init, time.time() - TIME_START))
