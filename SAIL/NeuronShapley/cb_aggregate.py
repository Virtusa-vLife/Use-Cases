...
np.random.seed(0)
# class of image example zebra
keys = sys.argv[1]
# Performance metrics example accuracy
metric = sys.argv[2]
#Number of image in input image folder
num_images = int(sys.argv[3])
adversarials = sys.argv[4]
print("#"*50,"Aggregation Start","#"*50)
print("keys:{},metric:{},num_images:{},adversarials:{}".format(keys,metric,num_images,adversarials))
# i=0
while True:
#     i+=1
...
#         print(adv)
        ...
        for key in keys.split(','):
            ...
            results = np.sort([saved for saved in tf.gfile.ListDirectory(cb_dir)
                               if 'agg' not in saved and '.h5' in saved])#results:['000000.h5' '000001.h5'...]
           
            squares, sums, counts = [np.zeros(len(players)) for _ in range(3)]
            #squares:[0. 0. 0. ... 0. 0. 0.],sums:[0. 0. 0. ... 0. 0. 0.],counts:[0...]
            
            max_vals, min_vals = -np.ones(len(players)), np.ones(len(players))
            #max_vals:[-1. -1. -1. ... -1. -1. -1.],min_vals:[1. 1. 1. ... ]
            # Loading stored data during neuron shapley algorithm from .h5 file of result
            for result in results:
                try:
                    with h5py.File(os.path.join(cb_dir, result), 'r') as foo:
                        mem_tmc = foo['mem_tmc'][:]#mem_tmc:[]
                        print('*'*5,'result:{},mem_tmc length(mem_tmc>0):{}'.format(result,len(mem_tmc[mem_tmc>0])))
                ...
                print('*'*5,'sums:{},squares:{},counts:{}'.format(sums,squares,counts))
...
#           thresh =0.01
            ...
            # Saving information about players, varience, shapley values,counting value
