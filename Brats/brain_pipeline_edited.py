...
#            return slice
#        else:
#            va=(slice - np.mean(slice)) / np.std(slice)
#            return va 
    ...
    def save_patient(self, reg_norm_n4, patient_num):
        ...
        elif reg_norm_n4 == 'test':
            for slice_ix in progress(xrange(155)):
                strip = self.slices_by_slice[slice_ix].reshape(1200, 240)
                if np.max(strip) != 0:
                    strip /= np.max(strip)
                io.imsave('Testing_PNG/{}_{}.png'.format(patient_num, slice_ix), strip)
        else:
            for slice_ix in progress(xrange(155)): # reshape to strip
                strip = self.normed_slices[slice_ix].reshape(1200, 240)
                if np.max(strip) != 0: # set values < 1
...
def save_labels(fns):
    ...
    Labels folder contain OT Scan slices of .mha files of each patient in HGG  folder
    '''
    progress.currval = 0
...
if __name__ == '__main__':
    ...
    # Train Data Labels and Data Generations
    #print("Train Labels start from here")
    #labels = glob('Training/HGG/**/*more*/**.mha')
    #print("labels path",labels)
    #save_labels(labels)
    #print("Train labels end here")
    #patients = glob('Training/HGG/**')
    #save_patient_slices(patients, 'reg')
    #Train data generation ends here    
    
    #Test data generation
    patients = glob('Testing/**/**')
    print("Test patients list",patients)
    #save_patient_slices(patients, 'test')
