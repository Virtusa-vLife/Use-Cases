...
import json
import h5py
from patch_library_edited_2 import PatchLibrary
...
from keras.layers.core import Merge,MaxoutDense
...
class SegmentationModel(object):
    ...
    #def __init__(self, n_epoch=1, n_chan=4, batch_size=128, loaded_model=False, architecture='single', w_reg=0.01, n_filters=[64,128,128,128], k_dims = [7,5,5,3], activation = 'relu'):
        ...
        '''
        #print("Inside SegmentationModel Constructor")
    ...
    def fit_model(self, X_train, y_train, X5_train = None, save=True):
        ...
        checkpointer = ModelCheckpoint(filepath="/home/ec2-user/cardinal/BRATS2015/models/check/bm_{epoch:02d}-{val_loss:.2f}.hdf5", verbose=1)
    ...
    def save_model(self, model_name):
        ...
        '''
        print("************************Inside save_model function****************")
        model = '{}.json'.format(model_name)
        weights = '{}.hdf5'.format(model_name)
        ...
        print("*******************Model Saved inside ",model_name)   
    ...
    def predict_image(self, test_img,model, show=False):
        '''
    ...
    def show_segmented_image(self, test_img,model, modality='t1c', show = False):
        '''
        ...
        segmentation = self.predict_image(test_img,model=model, show=False)
        
        # Image masking of patches
        ...
        mask1 = [1, 0.2, 0.2]
        mask2 = [1,1,0.25]
        mask3 = [0.35,0.75,0.25]
        mask4 = [0,0.25,0.9]

        # consolidating and reconstructing final image using above image mask
        for i in range(len(ones)):
            sliced_image[ones[i][0]][ones[i][1]] = mask1
        for i in range(len(twos)):
            sliced_image[twos[i][0]][twos[i][1]] = mask2
        for i in range(len(threes)):
            sliced_image[threes[i][0]][threes[i][1]] = mask3
        for i in range(len(fours)):
            sliced_image[fours[i][0]][fours[i][1]] = mask4
...
if __name__ == '__main__':
    ...
    #print("*****************************Model Createad************************ Going to fit model with values********************* ")
    
    #model.fit_model(X, y)
    #print("*****************Going to save model ****************************")
    #model.save_model('models/example')
    # Train Content Ends Here

    #print("Number of argument passed",len(sys.argv))
    if(len(sys.argv)>1):
        #print("Inside if")
        # Test Content Start here
        SegObj = SegmentationModel()
        print("\n \n*************** Going to load CNN model already build***************")
        loadedModel=SegObj.load_model_weights("models/example")
        print("\n \n************************** CNN Model Loaded  ****************************")
        tests = glob("Testing_PNG/{}".format(sys.argv[1]))
        #print("tests",tests)
        #test_sort = sorted(tests, key= lambda x: int(x[12:-4]))
        segmented_images = []
        #print("Testing Sorted**************************",test_sort)
        for index,content in enumerate(tests):
            #print("Going for Training_PNG Image no",content)
            print("Going to load test scans ")
            print("\n \nPredicting the output")
            io.imsave("Testing_result/scan_{}_output.png".format(index),SegObj.show_segmented_image(test_img=content,model=loadedModel))
        print("\n \n************************** Testing completed ****************************")
